export default {
  ...
  pages: {
    login: {
      ...
      assistantTitle: '连接助手',
      assistantDescription: '先选择你的连接场景，系统会给出推荐地址和排错提示。',
      gatewayHint: '这里填写的是 Gateway 的 WebSocket 地址，不是当前前端页面地址。',
      ...
      preflightTitle: '连接前检查',
      failureTitle: '连接诊断',
      connect: '连接',
      useRecommended: '使用推荐地址',
      inputTokenRequired: '请输入 Gateway Token',
      inputGatewayUrlRequired: '请输入 Gateway 地址',
      ...
      scenes: {
        local: {
          title: '本机开发',
          description: 'OpenClaw Web 和 Gateway 都跑在本机。',
          recommendedUrl: 'ws://127.0.0.1:18789',
          tip: '优先使用 127.0.0.1 或 localhost，最适合第一次连通验证。',
        },
        lan: {
          title: '局域网机器',
          description: '前端和 Gateway 在同一内网，但不在同一台机器。',
          recommendedUrl: 'ws://你的网关内网IP:18789',
          tip: '若页面不是 HTTPS，设备签名可能失败；优先用 localhost、HTTPS 或反向代理。',
        },
        public: {
          title: '公网域名 / HTTPS',
          description: '通过 HTTPS 域名访问 OpenClaw Web，并连接远程 Gateway。',
          recommendedUrl: 'wss://你的域名',
          tip: '公网场景推荐使用 WSS，并在 Gateway 中配置 allowedOrigins。',
        },
      },
      diagnostics: {
        copy: '复制',
        ...
        actions: {
          useWsScheme: '改成 ws:// 或 wss:// 地址',
          switchToWss: '改用 WSS 或 HTTPS 页面',
          useHttpsOrLocalhost: '改用 HTTPS、localhost 或 127.0.0.1 访问前端',
          listRequests: '查看待批准设备',
          approveDevice: '批准当前设备',
          copyAllowedOrigins: '复制 allowedOrigins 配置片段',
          copyTokenCommand: '复制获取 Token 命令',
          checkGatewayAddress: '检查 Gateway 地址和网络连通性',
        },
        items: {
          invalidScheme: {
            title: 'Gateway 地址协议不正确',
            summary: 'Gateway 地址必须以 ws:// 或 wss:// 开头。',
            detail: '例如本机使用 ws://127.0.0.1:18789，HTTPS 域名场景使用 wss://你的域名。',
          },
          httpsWsBlocked: {
            title: 'HTTPS 页面不能直接连接非本地 ws://',
            summary: '当前页面是 HTTPS，浏览器会阻止连接非 localhost 的明文 ws:// 地址。',
            detail: '请把 Gateway 地址改成 wss://，或者把前端切回 localhost/127.0.0.1 的本地调试地址。',
          },
          secureContext: {
            title: '当前页面不是安全上下文',
            summary: '设备鉴权依赖 crypto.subtle，浏览器通常只在 HTTPS 或 localhost/127.0.0.1 下提供该能力。',
            detail: '如果你现在是通过内网 IP 的 http:// 页面访问前端，请改用 HTTPS，或在本机通过 localhost 打开页面。',
          },
          pairing: {
            title: 'Gateway 需要先批准当前设备',
            summary: '这是 2026.02.14+ 默认启用的设备配对鉴权行为，首次连接时很常见。',
          },
          allowedOrigins: {
            title: 'Gateway 缺少 allowedOrigins 配置',
            summary: '当前浏览器来源 {origin} 尚未加入 Gateway 的 controlUi.allowedOrigins。',
          },
          token: {
            title: 'Token 可能无效或缺失',
            summary: '请确认你输入的是当前 Gateway 的有效 Token，而不是其他环境或旧值。',
          },
          network: {
            title: 'Gateway 地址不可达或握手超时',
            summary: '浏览器没能完成 WebSocket 握手，常见原因是地址写错、端口未开放、反向代理未配置或网关未启动。',
            detail: '优先检查 Gateway 是否监听在目标地址上，再确认端口、防火墙和代理配置。',
          },
          generic: {
            title: '连接失败，需要进一步检查',
            summary: '系统没有命中明确规则，请先核对 Gateway 地址、Token 和部署方式。',
            detail: '若是远程部署，优先检查 WSS、allowedOrigins、设备批准和网关日志。',
          },
        },
      },
    },
    ...
    channels: {
      ...
      channels: {
        qqbot: {
          label: 'QQ',
          description: '面向 QQ 群与私聊场景。根据官方文档，顶层需配置 appId / clientSecret / markdownSupport，同时还需要 accounts.default.enabled。',
        },
        feishu: {
          label: '飞书',
          description: '适用于飞书企业协作场景。当前 Gateway 实际使用顶层 appId / appSecret 配置。',
        },
        dingtalk: {
          label: '钉钉',
      ...
      accountConfigTitle: '默认账号配置',
      accountConfigHint: '根据官方文档，部分渠道除了顶层参数外，还需要配置 accounts.default 账号项。当前页面先按默认账号进行维护。',
      accountConfigHintFeishu: '飞书当前按 Gateway 实际返回的顶层 appId / appSecret 进行维护。',
      labels: {
        enabled: '启用渠道',
        defaultAccountEnabled: '启用默认账号',
      ...
      placeholders: {
        ...
        appId: '输入 App ID',
        clientId: '输入 Client ID',
        appSecret: '输入 App Secret',
        corpId: '输入 Corp ID',
        agentId: '输入 Agent ID',
        verificationToken: '输入 Verification Token',
        encryptKey: '输入 Encrypt Key',
        token: '输入 Token',
        encodingAesKey: '输入 Encoding AES Key',
        secret: '输入新值（留空不变更）',
      },
    ...
    memory: {
      ...
      success: {
        title: '模型配置已准备就绪',
        description: '已保存 {providerName}（{modelRef}）。建议现在去聊天页发一条测试消息，验证模型是否可用。',
        goChat: '去聊天验证',
      },
      advanced: {
        show: '展开高级设置',
        hide: '收起高级设置',
        hint: '高级设置主要用于输入类型等细节调整，新手可先保持默认。',
      },
      editor: {
    ...
    models: {
      ...
      quick: {
        ...
        setupOutcome: '会自动写入渠道、模型列表，并在当前还没有默认模型时自动设为默认。',
        configuredTag: '已配置',
        latestTag: '最新默认',
        providerId: '渠道 ID',
        defaultModel: '默认模型',
      ...
      form: {
        ...
        providerPresetPlaceholder: '选择主流厂商，或保持自定义',
        providerPresetHint: '选择后会自动填入 Provider ID、推荐协议与 Base URL；如使用中转站或私有代理，仍可继续手动修改。',
        providerPresetEditHint: '用于快速对齐推荐协议与 Base URL，不会修改现有 Provider ID；如使用中转站或私有代理，仍可继续手动修改。',
        providerPresetCustom: '自定义 / 中转站',
        providerId: '渠道 ID',
        ...
        assistantModelPreset: '助理模型预设',
        assistantModelPresetPlaceholder: '选择该厂商常用助理模型',
        assistantModelPresetHint: '这里只提供常用助理模型预设；选择后会自动填入模型 ID，并尽量同步推荐协议，你仍可继续手动改。',
        modelInputTypes: '输入类型',
        modelInputTypesPlaceholder: '选择该渠道模型支持的输入类型（可多选）',
        models: '模型列表',
        modelsPlaceholder: '可先从上方助理模型预设中选择，或手动填写单个模型 ID',
        detectedModels: '已识别',
        modelsCount: '共 {count} 个模型',
        presetTierRecommended: '推荐',
        presetTierPowerful: '更强',
        presetTierEconomy: '更省',
        presetTierLocal: '本地',
      },
      fields: {
        providerId: '渠道 ID',
      ...
      createModal: {
        title: '新建渠道',
        flowHint: '新建流程：选择厂商预设或手动填写 -> 选择助理模型 -> 查看保存预览 -> 创建渠道。',
      },
      messages: {
    ...
    dashboard: {
      ...
      quickstart: {
        title: '新手开始',
        subtitle: '按这个顺序走，最快把 OpenClaw 跑通。',
        status: {
          done: '已完成',
          next: '下一步',
          blocked: '前置未完成',
        ...
        tasks: {
          connection: {
            title: '连接 Gateway',
            pending: '先确认当前浏览器能稳定连上 Gateway。若失败，请根据连接诊断修复。',
            done: '当前网关已连接，可以继续完成后续配置。',
          },
          models: {
            title: '配置模型',
            pending: '至少配置一个可用模型，后续聊天和 Agent 才能正常工作。',
            done: '检测到已有模型配置，可以继续验证对话链路。',
          },
          chat: {
            title: '发送第一条消息',
            pending: '去聊天页发送一条测试消息，验证模型、会话和响应流都正常。',
            done: '已经检测到会话数据，说明基础链路大概率已经跑通。',
            blocked: '请先完成连接和模型配置，再去聊天页发起第一条消息。',
          },
        },
      },
      connection: {
